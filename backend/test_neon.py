...
def token(key, **updates):
    payload={'sub':'user-A','iss':'https://auth.example.test','aud':'https://auth.example.test',
...
@pytest.mark.parametrize('updates',[{'exp':1},{'iss':'https://attacker.test'},{'iss':'https://auth.example.test/auth'},{'aud':'other-project'},{'sub':''},{'role':'anonymous'},{'is_anonymous':True},{'iat':int(time.time())+120},{'nbf':int(time.time())+120}])
...
def test_neon_accepts_small_provider_clock_skew(signing):
    db=neon_auth.session('Bearer '+token(signing,iat=int(time.time())+12))
    assert db.user['id']=='user-A'

def test_neon_respects_explicit_issuer(signing,monkeypatch):
    monkeypatch.setenv('NEON_AUTH_ISSUER','https://custom-issuer.example.test')
    assert neon_auth.session('Bearer '+token(signing,iss='https://custom-issuer.example.test')).user['id']=='user-A'
