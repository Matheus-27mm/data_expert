...
test('workspace persists company, stock, bills, CSV and PDF; accessible on mobile',async({page})=>{
 ...
 await page.getByRole('button',{name:'Salvar movimentação',exact:true}).click();
 ...
 await page.getByRole('link',{name:'Pagar conta',exact:true}).first().click();
 await page.getByRole('button',{name:'Registrar pagamento',exact:true}).click();
 await page.getByRole('combobox',{name:'Conta',exact:true}).selectOption({label:'CONTA-001 · Aluguel'});
 await page.getByLabel('Referência do pagamento').fill('PAG-001');
 ...
 await page.getByLabel('Identificador único da venda').fill('MANUAL-001');
 await page.getByLabel('Produto',{exact:true}).fill('Caneca manual');
 await page.getByLabel('Categoria',{exact:true}).fill('Casa');
 await page.getByLabel('Receita total (R$)').fill('25');
 await page.getByLabel('CMV total (R$)').fill('10');
 await page.getByRole('button',{name:'Salvar venda',exact:true}).click();
 await expect(page.getByRole('cell',{name:'MANUAL-001',exact:true})).toBeVisible();
 await page.getByRole('textbox',{name:'Buscar nesta tabela'}).fill('MANUAL-001');
