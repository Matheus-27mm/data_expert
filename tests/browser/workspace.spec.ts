...
test('workspace persists company, stock, bills, CSV and PDF; accessible on mobile',async({page})=>{
 ...
 await page.getByLabel('Objetivo e próximos passos').fill('Aumentar a margem');
 await page.getByRole('button',{name:'Salvar plano'}).click();
 await page.getByRole('button',{name:/Revisar preços/}).click();
 const detail=page.getByRole('dialog',{name:'Acompanhar plano'});
 await expect(detail.getByRole('heading',{name:'Revisar preços',exact:true})).toBeVisible();
 await detail.getByLabel('Registrar atualização').fill('Preço revisado com a equipe.');
 await detail.getByRole('button',{name:'Adicionar ao histórico'}).click();
 await expect(detail.getByText('Preço revisado com a equipe.',{exact:true})).toBeVisible();
 await detail.getByLabel('Andamento').selectOption('done');
 await expect(detail.getByLabel('Andamento')).toHaveValue('done');
 await detail.getByRole('button',{name:'Fechar acompanhamento'}).click();
 await page.getByRole('link',{name:'Importações',exact:true}).first().click();
